...
export default function MCPConceptPage() {
  return (
    <div className="prose prose-lg max-w-none dark:prose-invert">
      ...
      <SectionTitle>🧩 Claude Use Case를 MCP 흐름에 연결하기</SectionTitle>
      <Paragraph>
        Claude 공식 use case 39개를 <strong>Cloud Ops</strong>와 <strong>General Biz</strong> 관점으로 재분류했습니다. 아래 대표 사례처럼
        <strong>SSO 프로파일</strong>, <strong>MCP 도구</strong>, <strong>사후 검증</strong>만 덧붙이면 기존 카드가 곧바로 Vibe Coding 워크플로가 됩니다.
        (전체 매핑은 <code className="font-mono">docs/reference/claude_use_case_tracks.csv</code> 참고)
      </Paragraph>

      <SubsectionTitle>Cloud Ops 대표 카드</SubsectionTitle>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-slate-900 text-slate-100 rounded-lg p-6 space-y-3">
          <div className="text-sm font-semibold text-blue-300">generate-project-status-reports</div>
          <p className="text-sm text-slate-200">
            <strong>Activation</strong>: 다중 고객사 프로젝트 현황을 주간으로 집계해야 할 때 (Delivery PM)
          </p>
          ...
          <ul className="text-xs space-y-1 list-disc pl-4 text-slate-200">
            <li>Git MCP로 Jira 스냅샷과 배포 로그를 동시에 읽기</li>
            <li>Amazon Q Developer CLI가 DOCS 템플릿으로 정리</li>
            <li>검증: Slack #wbr 채널에 공유 전, Git diff와 프로젝트 오너 승인 필수</li>
          </ul>
        </div>
        <div className="bg-slate-900 text-slate-100 rounded-lg p-6 space-y-3">
          <div className="text-sm font-semibold text-emerald-300">organize-your-business-finances</div>
          <p className="text-sm text-slate-200">
            <strong>Activation</strong>: 월간 FinOps 리뷰에서 AWS CUR/RI 사용률을 한 번에 보여줘야 할 때 (FinOps Lead)
          </p>
          <div className="bg-black/40 rounded p-3 text-xs font-mono space-y-1">
            <div className="text-green-400">AWS_PROFILE=finops-prod q mcp run \</div>
            <div className="text-white">  --resource s3://cur-bucket/2025-01.csv \</div>
            <div className="text-white">  --tool playwright.screenshot "FinOps 대시보드 증빙 캡처"</div>
          </div>
          <ul className="text-xs space-y-1 list-disc pl-4 text-slate-200">
            <li>Context7 MCP로 CUR 스키마 문서 연동 → 지표 정의 일치 여부 확인</li>
            <li>Amazon Q Developer CLI 리포트 템플릿에 KPI (RI, Savings Plan, CUD) 자동 채우기</li>
            <li>검증: AWS Cost Explorer 수치와 교차 검증, 재무팀 결재선 기록</li>
          </ul>
        </div>
      </div>

      <SubsectionTitle>General Biz 대표 카드</SubsectionTitle>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6 space-y-3">
          <div className="text-sm font-semibold text-purple-500 dark:text-purple-300">build-customer-personas</div>
          <p className="text-sm text-gray-600 dark:text-gray-300">
            <strong>Activation</strong>: 세일즈 킥오프 직전에 산업별 페르소나 카드가 필요할 때 (Marketing Ops)
          </p>
          <div className="bg-gray-900 text-gray-100 rounded p-3 text-xs font-mono space-y-1">
            <div className="text-green-400">AWS_PROFILE=revops-sso q mcp run \</div>
            <div className="text-white">  --resource fetch://crm-api/persona-export.json \</div>
            <div className="text-white">  --tool memory.write "업데이트된 페르소나 지식"</div>
          </div>
          <ul className="text-xs space-y-1 list-disc pl-4 text-gray-600 dark:text-gray-300">
            <li>CRM/인터뷰 노트를 Resource로 불러와 pain point/제안 메시지를 표준화</li>
            <li>Memory MCP에 저장해 Claude Code/Slack에서 재사용</li>
            <li>검증: 영업팀 2인 이상 리뷰, 최신 reference deck 링크 첨부</li>
          </ul>
        </div>
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6 space-y-3">
          <div className="text-sm font-semibold text-indigo-500 dark:text-indigo-300">create-new-hire-onboarding-guides</div>
          <p className="text-sm text-gray-600 dark:text-gray-300">
            <strong>Activation</strong>: 신규 입사자용 Day-1~Day-5 커리큘럼을 부서별로 맞춤 제작해야 할 때 (HR Ops)
          </p>
          <div className="bg-gray-900 text-gray-100 rounded p-3 text-xs font-mono space-y-1">
            <div className="text-green-400">AWS_PROFILE=hr-portal q mcp run \</div>
            <div className="text-white">  --resource filesystem://checklist/hr-kit.md \</div>
            <div className="text-white">  --tool playwright.pdf "Notion 템플릿을 PDF로 변환"</div>
          </div>
          <ul className="text-xs space-y-1 list-disc pl-4 text-gray-600 dark:text-gray-300">
            <li>Filesystem MCP로 기존 온보딩 체크리스트/정책 문서를 읽어 템플릿 자동 채우기</li>
            <li>Playwright MCP PDF 출력 → People Portal 업로드</li>
            <li>검증: 개인정보/보안 교육 링크가 최신인지 HR Ops 체크</li>
          </ul>
        </div>
      </div>
