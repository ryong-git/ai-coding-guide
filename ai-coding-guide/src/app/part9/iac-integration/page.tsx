...
AWS_PROFILE=bespin-infra-admin q chat --no-interactive $'당신은 AWS 솔루션 아키텍트입니다.\nap-northeast-2 리전에 EKS + RDS + ALB로 구성된 3-tier 프로덕션 환경을\nterraform 모듈 구조(main.tf, variables.tf, outputs.tf)로 작성하세요.\n보안 그룹은 최소 권한, 모든 리소스에는 Environment=prod 태그를 붙여주세요.'`}</pre>
      ...
      <p>
        ...
        <pre className="bg-gray-900 text-gray-100 rounded-lg p-4 overflow-x-auto whitespace-pre-wrap text-sm font-mono mb-4">{`# Plan → AI 리뷰
...
terraform apply -auto-approve tfplan.binary</div>
      ...
      <div className="space-y-3">
        <pre className="bg-gray-900 text-gray-100 rounded p-3 text-sm font-mono overflow-x-auto whitespace-pre-wrap">{`# Change Set 분석
aws cloudformation create-change-set \
  --stack-name prod-network --change-set-name preview \
  --template-body file://network.yaml
aws cloudformation describe-change-set \
  --stack-name prod-network --change-set-name preview \
  > change-set.json
AWS_PROFILE=bespin-infra-admin q chat --no-interactive $'CloudFormation Change Set 내용을 검토해\n서비스 중단 위험과 IAM 권한 변경 사항을 표로 정리해줘.'`}</pre>
      </div>
